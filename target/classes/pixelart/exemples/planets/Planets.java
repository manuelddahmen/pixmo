...
/*     */ public class Planets
    ...
    /*     */
    /*     */
    public Planets(String dossier, String prefix, int largeur, int hauteur, boolean isGui) {
        /*  25 */
    ...
    public static void main(String[] args) {
        ...
        /* 106 */
        Planets c = new Planets(path, "im2-", 1388, 768, isGui);
        /* 107 */
        c.init(100);
        /*     */
        /* 109 */
    ...
    public void initImage() {
        ...
        for (int i = 1; i < this.numElectrons + 1; i++) {
            ...
            /*  70 */
            this.dx[i] = this.dx[i] + -1000.0D * this.poids[0] * Math.signum((this.x[i] - this.x[0])) / ((
                    /*  71 */         this.x[i] - this.x[0]) * (this.x[i] - this.x[0]) + (this.y[i] - this.y[0]) * (
                    /*  72 */         this.y[i] - this.y[0]));
            /*  73 */
            this.dy[i] = this.dy[i] + -1000.0D * this.poids[0] * Math.signum((this.y[i] - this.y[0])) / ((
    ...
    public void dessiner() {
        ...
        g2.clearRect(0, 0, this.largeur, this.hauteur);
        /*  89 */
        g2.setColor(Color.red);
        /*  90 */
        g2.fillOval(this.x[0], this.y[0], 30, 30);
        /*  91 */
        for (int i = 1; i < this.poids.length; i++) {
            /*  92 */
            g2.setColor(Color.blue);
            /*  93 */
            g2.fillOval(this.x[i], this.y[i], 10, 10);
            /*     */
        }
        /*     */
    }
    ...
    public void init(Hashtable<String, Object> objects) {
        /* 123 */
        init(((Integer) objects.get("numElectrons")).intValue());
        /*     */
    }
    /*     */
}
