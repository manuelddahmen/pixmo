...
public class Planets2
    ...
    public static void main(String[] args) {
        ...
        /* 139 */
        Planets2 c = new Planets2(path, "im2-", 1388, 768, isGui);
        /* 140 */
        c.init(100);
        /*     */
        /* 142 */
        ...
        for (int i = 1; i < this.poids.length; i++) {
            ...
            for (int h = 0; h < this.historySize; h++) {
                ...
                if (this.history[i][h] != null)
                    /*  99 */
                    dessinerForme((int) this.history[i][h].getX(), (int) this.history[i][h].getY(), this.historySize - h);
                /*     */
            }
            /*     */
        }
            ...
            for (int j = 0; j < d * 2; j++) {
                ...
                if (x0 - d + i >= 0 && x0 - d + i < this.largeur && y0 - d + j >= 0 &&
                    ...
                    float d0 = (float) Math.sqrt(((i - d) * (i - d) + (j - d) * (
                            /* 109 */               j - d)));
                    /* 110 */
                    int c1 = this.image.getRGB(x0 - d + i, y0 - d + j);
                    /* 111 */
                    int c = 16777215;
                    /* 112 */
                    int c2 = 0;
                    /* 113 */
                    if ((c1 >> 16 & 0xFF) + 0.0D < ((c >> 16 & 0xFF) *
                            /* 114 */             d0 / d)) {
                        /* 115 */
                        c2 += c1 & 0xFF0000;
                        /*     */
                    ...
                    if ((c1 >> 8 & 0xFF) + 0.0D < ((c >> 8 & 0xFF) * d0 / d)) {
                        /* 119 */
                        c2 += c1 & 0xFF00;
                        /*     */
                    } else {
                    ...
                    this.image.setRGB(x0 - d + i, y0 - d + j, c2);
                    /*     */
                }
                /*     */
            }
    ...
    public void init(Hashtable<String, Object> objects) {
        /* 157 */
        init(((Integer) objects.get("nombre")).intValue());
        /*     */
    }
    /*     */
}
